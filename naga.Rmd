...
plastic2 <- plastic %>%
  ...
  mutate(`Coastal Population Proportion` = ifelse(`Coastal Population` / `Total Population` < 1, `Coastal Population` / `Total Population`, 1)) %>% # when coastal population > total population, we consider the entire population to be coastal (islands) -> every point in a country might be less than 50 km from a coastline
  mutate(`Per Capita GDP (in thousands)` = `Per Capita GDP` / 1000) %>%
  dplyr::select(Country, `Per Capita GDP`, `Per Capita GDP (in thousands)`, `Coastal Population Proportion`, `Coastal Population`, `Total Population`, `Mismanaged Waste Proportion`)
```
...
ggplot(data = plastic2, mapping = aes(x = `Per Capita GDP (in thousands)`, y = `Mismanaged Waste Proportion`)) +
  geom_point() +
  labs(x = "PPP GDP Per Capita (in thousands)", y = "Proportion of Mismanaged Plastic Waste", title = "Response vs. PPP GDP Per Capita")
```

```{r include = T, fig.width = 4.5, fig.height = 3, fig.cap = "The Relationship Between PPP GDP Per Capita (in thousands) and Mismanaged Plastic Waste Proportion (after log-transformation)"}
ggplot(data = plastic2, mapping = aes(x = log(`Per Capita GDP (in thousands)`), y = `Mismanaged Waste Proportion`)) +
  geom_point() +  
  labs(x = "Log(PPP GDP Per Capita)", y = "Proportion of Mismanaged Plastic Waste", title = "Response vs. PPP GDP Per Capita")
```

# log-transform per capita GDP -> varying orders of magnitude
...
plastic2 %>%
  ...
  slice(1:5) %>%
  dplyr::select(Country, `Mismanaged Waste Proportion`)
```
...
- need info about response to continue
...
```{r}
bm1 <- betareg(`Mismanaged Waste Proportion` ~ `Per Capita GDP (in thousands)`, data = plastic2)

bm2 <- betareg(`Mismanaged Waste Proportion` ~ `Per Capita GDP (in thousands)` + `Coastal Population Proportion`, data = plastic2)

lm1 <- lm(`Mismanaged Waste Proportion` ~ log(`Per Capita GDP (in thousands)`), data = plastic2)
# mixture component at zero and at one - all assumptions for parametric regression models 

lm2 <- lm(log(`Mismanaged Waste Proportion`) ~ `Per Capita GDP (in thousands)` + `Coastal Population Proportion`, data = plastic2)
```

```{r}
zinfb1 <- gamlss(`Mismanaged Waste Proportion` ~ `Per Capita GDP (in thousands)` + `Coastal Population Proportion`, family = BEZI, data = plastic2, trace = F)
```
...
```{r include = T}
rownames(final_output) <- c("Intercept", "PPP GDP Per Capita (in thousands)", "Coastal Population Proportion")
